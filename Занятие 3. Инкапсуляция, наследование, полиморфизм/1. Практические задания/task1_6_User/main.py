class User:
    def __init__(self, username: str, password: str, address: str):
        ...
        self.__adress = address  # TODO создайте приватные атрибуты для пароля и адреса

    @staticmethod
    def check_permission(role: str) -> bool:
        """Простая проверка роли для доступа к конфиденциальным данным"""
        return role == "admin" # TODO если роль 'admin', то вернуть True

    def get_password(self, role: str) -> str:
        if self.check_permission(role):
            return self.__password
        else:
            raise PermissionError("Доступ запрещен")  # TODO провести проверку доступа по роли, если прошла, то вернуть пароль, если не прошел проверку, то вызвать исключение PermissionError с текстом 'Доступ запрещен'
